...
bool has_issue(const dpor::model::ConsistencyResult& result,
               const dpor::model::ConsistencyIssueCode code) {
  return std::any_of(
      result.issues.begin(), result.issues.end(),
      [code](const dpor::model::ConsistencyIssue& issue) { return issue.code == code; });
}
}  // namespace
...
TEST_CASE("event helper predicates classify labels", "[model][event]") {
  const dpor::model::Event receive{
      .thread = 1,
      .index = 0,
      .label = dpor::model::make_receive_label_from_values<dpor::model::Value>({"v1", "v2"}),
  };
...
TEST_CASE("execution graph tracks po and rf relations", "[model][graph]") {
  ...
  const auto recv_id = graph.add_event(
      2, dpor::model::make_receive_label_from_values<dpor::model::Value>({"x", "y"}));
...
TEST_CASE("execution graph stores bottom reads-from without consuming sends", "[model][graph]") {
  ...
  const auto recv_id =
      graph.add_event(2, dpor::model::make_nonblocking_receive_label<dpor::model::Value>());
...
TEST_CASE("async consistency checker accepts well-formed graph", "[model][consistency]") {
  ...
  const auto recv_id =
      graph.add_event(2, dpor::model::make_receive_label_from_values<dpor::model::Value>({"ok"}));
...
TEST_CASE("async consistency checker reports invalid reads-from endpoints",
          "[model][consistency]") {
  dpor::model::ExecutionGraph graph;
  const auto send_id = graph.add_event(1, dpor::model::SendLabel{.destination = 2, .value = "x"});
  const auto recv_id =
      graph.add_event(2, dpor::model::make_receive_label_from_values<dpor::model::Value>({"x"}));
...
TEST_CASE("async consistency checker requires every receive to read exactly one send",
          "[model][consistency]") {
  dpor::model::ExecutionGraph graph;
  const auto recv_id =
      graph.add_event(2, dpor::model::make_receive_label_from_values<dpor::model::Value>({"x"}));
...
TEST_CASE("async consistency checker reports destination and value mismatches",
          "[model][consistency]") {
  dpor::model::ExecutionGraph graph;
  const auto send_bad_dst =
      graph.add_event(1, dpor::model::SendLabel{.destination = 7, .value = "x"});
  const auto recv_bad_dst =
      graph.add_event(2, dpor::model::make_receive_label_from_values<dpor::model::Value>({"x"}));
  const auto send_bad_value =
      graph.add_event(1, dpor::model::SendLabel{.destination = 3, .value = "y"});
  const auto recv_bad_value =
      graph.add_event(3, dpor::model::make_receive_label_from_values<dpor::model::Value>({"x"}));
...
TEST_CASE("async consistency checker reports sends consumed by multiple receives",
          "[model][consistency]") {
  dpor::model::ExecutionGraph graph;
  const auto send_id = graph.add_event(1, dpor::model::SendLabel{.destination = 2, .value = "x"});
  const auto recv_0 =
...
TEST_CASE("async consistency checker reports causal cycles", "[model][consistency]") {
  ...
  const auto recv_t1 = graph.add_event_with_index(
      1, 0, dpor::model::make_receive_label_from_values<dpor::model::Value>({"b"}));
  const auto send_t1 =
      graph.add_event_with_index(1, 1, dpor::model::SendLabel{.destination = 2, .value = "a"});
  const auto recv_t2 = graph.add_event_with_index(
      2, 0, dpor::model::make_receive_label_from_values<dpor::model::Value>({"a"}));
  const auto send_t2 =
      graph.add_event_with_index(2, 1, dpor::model::SendLabel{.destination = 1, .value = "b"});
...
TEST_CASE("execution graph supports custom value type", "[model][graph]") {
  ...
  const auto send_id = graph.add_event(
      1, dpor::model::SendLabelT<Payload>{.destination = 2, .value = Payload{.id = 7}});
  const auto recv_id =
      graph.add_event(2, dpor::model::make_receive_label<Payload>(
...
TEST_CASE("custom message payload keeps typed fields", "[model][graph][consistency]") {
  ...
  const auto send_id = graph.add_event(3, dpor::model::SendLabelT<Message>{
                                              .destination = 4,
                                              .value = Message{.kind = "append_entries", .term = 9},
                                          });
  const auto recv_id =
      graph.add_event(4, dpor::model::make_receive_label<Message>([](const Message& candidate) {
