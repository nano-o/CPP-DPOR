...
[[nodiscard]] inline Options parse_args(
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--max-send-revisits-remote") {
      options.parallel = true;
      options.parallel_options.max_send_revisits_remote = parse_positive_int(value, arg);
      continue;
    }
  ...
  try {
    ...
    if (options.parallel && options.mode != detail::Options::Mode::Oracle) {
      std::cout << " parallel=true"
                ...
                << " max_send_revisits_remote="
                << options.parallel_options.max_send_revisits_remote
